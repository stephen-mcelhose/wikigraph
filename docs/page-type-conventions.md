...
description: Proposal defining required sections, tone/audience guidance, frontmatter schemas, progressive discovery via AGENTS.md, and proposal/spike retention strategy.
...
The wiki uses six page types (`concept`, `how-to`, `decision`, `runbook`, `proposal`, `spike`), but structural requirements were previously informal. This page defines exact section schemas, tone/audience guidelines, required frontmatter fields, progressive discovery mechanics, and retention policies.
...
## Required Sections & Tone Guidance by Page Type

| Page Type | Required Sections (Canonical Order) | Tone & Audience |
| :--- | :--- | :--- |
| **`decision`** | 1. `## Context`<br>2. `## Decision`<br>3. `## Consequences`<br>4. `## Sources` | **Objective, analytical**. Audience: developers/architects seeking rationale. Omit markdown `## Status`. `## Decision` starts with Y-statement (`In the context of <X>, facing <Y>, we decided <Z>, to achieve <A>, accepting <B>`). |
| **`how-to`** | 1. `## Goal`<br>2. `## Prerequisites`<br>3. `## Steps`<br>4. `## Verification`<br>5. `## Troubleshooting`<br>6. `## Sources` | **Imperative, task-focused**. Audience: end users executing a task. Must satisfy 24-point how-to rubric. |
| **`concept`** | 1. `## Overview`<br>2. `## Key Properties`<br>3. `## Related Concepts`<br>4. `## Sources` | **Explanatory, domain-oriented**. Audience: reader learning Markov chains or graph concepts. Narrative freedom within minimal skeleton. |
| **`proposal`** | 1. `## Problem`<br>2. `## Proposed Solution`<br>3. `## Alternatives Considered`<br>4. `## Open Questions`<br>5. `## Sources` | **Persuasive, trade-off oriented**. Audience: team evaluating architectural options before consensus. |
| **`spike`** | 1. `## Question`<br>2. `## Method`<br>3. `## Findings`<br>4. `## Conclusions`<br>5. `## Sources` | **Empirical, evidence-based**. Audience: engineers reviewing test data, POC code results, or performance benchmarks. |
| **`runbook`** | 1. `## Overview`<br>2. `## Prerequisites`<br>3. `## Test Cases`<br>4. `## Sources` | **Procedural, assertion-heavy**. Audience: QA/maintainers executing manual or automated verification runs. |
